...
describe('Cart Component Integration Tests', () => {
  ...
  describe('a) Test rendering và user interactions', () => {
    ...
    test('TC2: Hiển thị giỏ hàng rỗng khi API không trả mảng items', async () => {
      vi.mocked(cartService.getCart).mockResolvedValue({
        success: true,
      });
    ...
    test('TC3: Hiển thị danh sách sản phẩm trong giỏ hàng', async () => {
      // Arrange
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
    ...
    test('TC4: Xóa sản phẩm khỏi giỏ hàng', async () => {
      // Arrange
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
    ...
    test('TC5: Xóa sản phẩm xử lý response không có items', async () => {
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
            productId: 'P001',
    ...
    test('TC6: Cập nhật số lượng sản phẩm trong giỏ', async () => {
      // Arrange
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
    ...
    test('TC7: Cập nhật số lượng xử lý response không có items', async () => {
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
            productId: 'P001',
    ...
    test('TC8: Hiển thị loading state khi tải giỏ hàng', async () => {
      // Arrange
    ...
    test('TC9: Không set state nếu unmount trước khi getCart resolve', async () => {
    ...
    test('TC10: Không set state nếu unmount trước khi getCart reject', async () => {
  ...
  describe('b) Test form submission và API calls', () => {
    test('TC11: Checkout yêu cầu địa chỉ, gọi API đặt hàng thành công và xóa giỏ', async () => {
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
            productId: 'P001',
  ...
  describe('c) Test error handling và success messages', () => {
    test('TC12: Cập nhật số lượng hiển thị lỗi khi vượt tồn kho', async () => {
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
            productId: 'P001',
    ...
    test('TC13: Hiển thị thông báo lỗi khi API gặp sự cố', async () => {
      // Arrange
    ...
    test('TC14: Chặn số lượng không hợp lệ khi cập nhật giỏ', async () => {
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
            productId: 'P001',
    ...
    test('TC15: Áp dụng coupon hiển thị message hợp lệ và không hợp lệ', async () => {
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
            productId: 'P001',
    ...
    test('TC16: Checkout hiển thị lỗi khi tạo đơn thất bại', async () => {
      vi.mocked(cartService.getCart).mockResolvedValue({
        items: [
          {
            productId: 'P001',
